...
export const productsByCategory = async (request, response) => {
    const ctg = request.params.ctg
    const data = await products.find({ ctg })
    const filter = {title: data.title, desc: data.desc, img: data.img, id: data._id, price: data.price}
...
export const createProduct = async (request, response) => {
    ...
    const { title, desc, ctg, price } = request.body;
    ...
    try {
        await waiting.create({title, desc, price, ctg, img : imgUrl, file : fileUrl, by: user.username, id: randomize()})
        img.mv(imgPath)
        file.mv(filePath)
...
export const confirmProduct = async (request, response) => {
    ...
    try {
        await products.create({price : data.price, title : data.title, desc : data.desc,file : data.file, img : data.img,ctg : data.ctg, by: data.by,})
